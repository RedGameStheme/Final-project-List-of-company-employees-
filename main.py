...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email', 'zp'),
        ...
        self.tree.heading("zp", text='Зароботная плата')
    ...
    def update_record(self, name, tel, email, zp):
        self.db.c.execute('''UPDATE db SET name=?, tel=?, email=?, zp=? WHERE ID=?''',
                          (name, tel, email, zp, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_sum = tk.Label(self, text='ЗП')
        ...
        # добавляем строку ввода для зароботной платы
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, zp integer)''')
    ...
    def insert_data(self, name, tel, email, zp):
        self.c.execute('''INSERT INTO db (name, tel, email, zp) VALUES (?, ?, ?, ?)''',
                       (name, tel, email, zp))
        self.conn.commit()
